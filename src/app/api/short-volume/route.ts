...
export async function GET(req: Request) {
  try {
    ...
    const url = `https://api.massive.com/stocks/v1/short-volume?ticker=${encodeURIComponent(ticker)}&limit=${encodeURIComponent(limit)}&sort=settlement_date.desc${apiKey ? `&apiKey=${encodeURIComponent(apiKey)}` : ''}`;
